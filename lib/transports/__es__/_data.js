...
class Data {
  ...
  setData (data, limit, offset, callback) {
    ...
    const MaxRequestSize = bytes(this.parent.options.fileSize)
    const fileSize       = this.parent.options.fileSize
    let _callback = null;
    ...
    data.forEach(elem => {
      ...
      let bodyActionMeta = `${jsonParser.stringify(actionMeta, this.parent)}
`
      let bodyElemSource = `${jsonParser.stringify(elem._source, this.parent)}
`
    
      if (fileSize != 0) {
        let newPlayload = JSON.stringify(payload) + bodyActionMeta + bodyElemSource;
        let byteResult  = Buffer.byteLength(newPlayload, 'utf8');
        
        if (payload.body.length > 0 && byteResult > MaxRequestSize) {
          // this.parent.emit('log', `Payload Size: (${byteResult}) is over ${MaxRequestSize}, at ${elem._index} No. ${indexCount}. Splitting request`);
          // lasOfSe + indexCount - 1
          // this.parent.emit('log', `Splitting request into: ${}/${}`)
          // payloadOffset.push(`${lastOffSet}/${lastOffSet + indexCount}`)
          this.parent.emit('log', `Splitting request into: ${lastOffSet}/${offset + indexCount - 1}`)
          lastOffSet = lastOffSet + indexCount - 1;
      ...
      payload.body += bodyActionMeta
      payload.body += bodyElemSource
    });
    
    if (payload.body.length > 0) {
      
      payloads.push(payload);
      if (payloads.length > 1) { this.parent.emit('log', `Splitting request into: ${lastOffSet}/${offset + limit}`); }
    }
    
    // if (payloads.length > 1) { this.parent.emit('log', `Splitting ${limit} objects into ${payloads.length} requests.`); }
    
    const sendRequest = (_payload) => {
      return new Promise(async (resolve, reject) => {
        // TODO update log to have offset + 
        // this.parent.emit('log', `Sent - writes: ${writes}`)
        this.baseRequest(_payload, (err, response) => {
          err = this.handleError(err, response)
          if (err) {
            return callback(err, [])
          }
          // this.parent.emit('log', `sendRequest - writes: ${writes}`)
    ...
    const sendRequests = async () => {
      const status = await Promise.all(payloads.map(async _payload => await sendRequest(_payload))).then( status => {
        sendReindexRequest()
      });
    }
